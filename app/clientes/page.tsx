  ...
  const sidebar = (
    <div style={{ width: '200px', background: '#1a1a2e', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
      ...
      {[
        ...
        <a key={item.href} href={item.href} style={{ display: 'block', padding: '9px 20px', fontSize: '13px', color: item.href === '/clientes' ? '#C9A84C' : '#8888aa', textDecoration: 'none', margin: '0 8px', borderRadius: '8px', background: item.href === '/clientes' ? '#C9A84C18' : 'transparent' }}>
          {item.label}
        </a>
      ))}
    </div>
